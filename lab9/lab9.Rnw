...
\section{Abstract}
The purpose of this report is to apply the computational tools that we have learned about in class to reproduced a simple regression analysis. Specifically, we are trying to reproduce the analysis that was done in section 3.1 of \emph{An Introduction of Statistical Learning} by Gareth James, Daniela Witten, Trevor Hastie and Robert Tibshirani. 
...
\section{Method}
We look at the data set and study the relationship between TV advertising budget and sales. This statement helps us to set up our null and alternate hypothesis. Our null hypothesis is there is no relationship between TV advertising budget and sales. Our alternate hypothesis is, there is a relationship between TV advertising budget and sales. These are equal to the following 
...
print(tbl, caption.placement = 'top',
      comment = getOption("xtable.comment", FALSE), type = 'latex')

@

In table 1, we are given a lot of information about our least squares model. Here we can see that beta0 hat is 7.03 and beta1 hat is .05. Being mindful of the units of each variable, what this means is that for every additional \$1000 spent on TV advertising there will be 50 additional units of that product sold. Focusing on 

\begin{equation}
 \hat{\beta_1} = 0.05
\end{equation}

we can see that the standard error for this is rounds to 0, so the standard error is very small. So, the chances that beta1 equaling 0 is very small. Also, if we take into account the large t-statistic, then we have enough evidence to reject our null hypothesis. So, there is a relation between TV advertising budget and sales. 

We have also provided more information about the least squares model in table 2.

<<Regression_Quality, results= "asis", echo = FALSE>>=
...
\begin{figure}
\includegraphics[width=1\textwidth]{images/scatterplot-tv-sales.png}
